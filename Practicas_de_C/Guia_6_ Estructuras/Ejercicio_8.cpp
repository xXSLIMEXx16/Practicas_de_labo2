/*Agrega una funci´on que permita buscar una persona por su nombre en el vector
y que imprima su informaci´on si se encuentra*/
...
void buscar_nombre(struct persona alumno[], int tamano){
    ...
    if (tamano != 0)
    {
        ...
        for (int i = 0; i < tamano; i++){
            if (strcmp(alumno[i].nombre, nombre_buscar) == 0){
                printf("=========== PERSONA ===========\n");
                printf("Nombre: %s\n",alumno[i].nombre);
                printf("Edad: %d\n",alumno[i].edad);
                printf("Genero: %c\n",alumno[i].Genero);
